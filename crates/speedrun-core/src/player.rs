...
impl Player {
    ...
    ///
    /// Valid range: 0.25 to 30.0. Values outside the range are clamped.
    /// NaN and negative-infinite values reset to 1.0.
    /// Positive-infinite values are clamped to the maximum (30.0).
    pub fn set_speed(&mut self, speed: f64) {
        if speed.is_nan() || speed == f64::NEG_INFINITY {
            self.speed = 1.0;
        } else {
            self.speed = speed.clamp(0.25, 30.0);
        }
    }
...
"#;
    ...
    fn test_set_speed_clamp_high() {
        let mut player = load_file("minimal_v2.cast");
        player.set_speed(35.0);
        assert_f64_eq(player.speed(), 30.0);
    }
